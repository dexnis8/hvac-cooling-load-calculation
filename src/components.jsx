...
export function LoadChart({ result, month, unit = 'kW' }) {
  ...
  const height = 230, left = unit === 'Btu/hr' ? 70 : 39, top = 15, bottom = 34, right = 19
  const tickStep = 10 ** Math.floor(Math.log10(result.peak.total || 1)) / 2
  const max = Math.max(tickStep, Math.ceil(result.peak.total / tickStep) * tickStep)
  ...
  return <section className="panel profile-panel"><div className="panel-heading"><div><h2>Hourly load profile</h2><p>{month} design day · All zones</p></div><div className="segmented" aria-label="Profile display">{['Chart', 'Table'].map(item => <button key={item} aria-pressed={view === item} onClick={() => setView(item)}>{item}</button>)}</div></div>{view === 'Chart' ? <><div className="chart-key"><span><i className="line-key total" />Total</span><span><i className="line-key envelope" />Sensible</span><span><i className="line-key latent dashed" />Latent</span><span className="chart-unit">Load ({unit})</span></div><div className="chart" ref={chartRef}><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Hourly cooling load. Peak ${number(result.peak.total)} ${unit} at ${time(result.peak.hour)}. Exact values available in Table view.`}>
    {Array.from({ length: 5 }, (_, i) => max * i / 4).map(v => <g key={v}><line x1={left} x2={width - right} y1={y(v)} y2={y(v)} className="grid-line" /><text x={left - 10} y={y(v) + 4} textAnchor="end">{number(v, max < 4 ? 2 : max < 40 ? 1 : 0)}</text></g>)}
    ...
    {result.hours.map(h => <rect key={h.hour} x={x(h.hour) - 10} y={top} width="20" height={height - top - bottom} fill="transparent" onMouseEnter={() => setActive(h.hour)}><title>{time(h.hour)} · {number(h.total)} {unit} total</title></rect>)}
  </svg></div><div className="chart-footer"><span><span className="status-dot" />Peak at <b className="mono">{time(result.peak.hour)}</b></span><label className="chart-inspect">Inspect hour <select aria-label="Inspect hourly load" value={active ?? result.peak.hour} onChange={e => setActive(Number(e.target.value))}>{result.hours.map(h => <option key={h.hour} value={h.hour}>{time(h.hour)}</option>)}</select><output className="mono" aria-live="polite">{number(selected.total)} {unit}</output></label></div></> : <div className="table-scroll chart-table"><table><caption className="sr-only">Exact hourly load values in {unit}</caption><thead><tr><th>Hour</th><th className="numeric">Sensible ({unit})</th><th className="numeric">Latent ({unit})</th><th className="numeric">Total ({unit})</th></tr></thead><tbody>{result.hours.map(h => <tr key={h.hour}><td className="mono">{time(h.hour)}</td><td className="numeric">{number(h.total - h.latent)}</td><td className="numeric">{number(h.latent)}</td><td className="numeric">{number(h.total)}</td></tr>)}</tbody></table></div>}</section>
}
export function Breakdown({ result, unit = 'kW' }) {
  const sorted = [...components].sort((a, b) => result.peak[b[0]] - result.peak[a[0]])
  return <section className="panel breakdown-panel"><div className="panel-heading"><div><h2>Load breakdown</h2><p>At coincident peak · {time(result.peak.hour)}</p></div><span className="small-unit mono">{unit}</span></div><table className="breakdown-table"><caption className="sr-only">Load components at coincident peak, {unit} and percentage</caption><thead className="sr-only"><tr><th>Component</th><th>Load ({unit})</th><th>Share</th></tr></thead><tbody>{sorted.map(([key, label]) => <tr key={key}><th scope="row"><div className="bar-label"><i style={{ background: `var(--color-${key})` }} />{label}</div><div className="bar-track"><div style={{ background: `var(--color-${key})`, width: `${result.peak.total ? result.peak[key] / result.peak.total * 100 : 0}%` }} /></div></th><td className="numeric">{number(result.peak[key])}</td><td className="numeric share">{number(result.peak.total ? result.peak[key] / result.peak.total * 100 : 0, 0)}%</td></tr>)}</tbody></table><div className="breakdown-total"><strong>Total unadjusted load</strong><span className="mono">{number(result.peak.total)} <small>{unit}</small></span></div></section>
}
